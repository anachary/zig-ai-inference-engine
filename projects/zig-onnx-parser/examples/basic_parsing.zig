...
pub fn main() !void {
    ...
    std.log.info("  Max model size: {}MB", .{config.max_model_size_mb});
    std.log.info("  Strict validation: {}", .{config.strict_validation});
    std.log.info("  Supported opset versions: {}-{}", .{ config.min_opset_version, config.max_opset_version });
    ...
    const test_model_data = createTestONNXModel(allocator) catch |err| {
        std.log.warn("Could not create test model: {}", .{err});
        return;
    };
    ...
    std.log.info("Model Inputs ({}):", .{inputs.len});
    for (inputs, 0..) |input, i| {
        std.log.info("  [{}] {s}: shape {any}, type {}", .{ i, input.name, input.shape, input.dtype });
        if (input.isFullyDefined()) {
            if (input.numel()) |elements| {
                std.log.info("      Total elements: {}", .{elements});
            }
        } else {
    ...
    std.log.info("Model Outputs ({}):", .{outputs.len});
    for (outputs, 0..) |output, i| {
        std.log.info("  [{}] {s}: shape {any}, type {}", .{ i, output.name, output.shape, output.dtype });
    }
    ...
    std.log.info("  Total nodes: {}", .{model.graph.nodes.items.len});
    ...
    while (op_iter.next()) |entry| {
        std.log.info("    {s}: {} nodes", .{ entry.key_ptr.*, entry.value_ptr.* });
    }
    ...
    const topo_order = model.graph.getTopologicalOrder(allocator) catch |err| {
        std.log.warn("Could not compute topological order: {}", .{err});
        return;
    };
    defer allocator.free(topo_order);
    
    std.log.info("Execution order ({} nodes):", .{topo_order.len});
    for (topo_order[0..@min(5, topo_order.len)], 0..) |node_idx, i| {
        const node = model.graph.nodes.items[node_idx];
        std.log.info("  [{}] {s} ({s})", .{ i, node.name, node.op_type });
    }
    if (topo_order.len > 5) {
    ...
    if (nodes_before != nodes_after) {
        std.log.info("Optimization removed {} nodes ({} -> {})", .{ nodes_before - nodes_after, nodes_before, nodes_after });
    } else {
    ...
    std.log.info("Default ONNX version: {}", .{onnx_parser.version.default_onnx_version});
